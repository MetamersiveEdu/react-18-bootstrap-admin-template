  ...
  function getServerChartOptions() {
  	...
  	return {
    	...
    	dataLabels: { enabled: false },
			grid: { show: true, borderColor: 'rgba(255, 255, 255, .15)' },
			stroke: { show: false },
    	colors: ['rgba(255, 255, 255, .25)', themeColor],
			...
			yaxis: { 
				labels: { style: { colors: '#ffffff', fontSize: '12px', fontFamily: themeFont, fontWeight: 400, cssClass: 'apexcharts-xaxis-label' } }
			},
			fill: { opacity: .65 },
									...
									<div className="col-lg-6 mb-3 mb-lg-0" key={index}>
										<div className="d-flex">
											...
											<div className="ps-3 flex-1">
												<div className="fs-10px fw-bold text-white text-opacity-50 mb-1">{server.name}</div>
												<div className="mb-2 fs-5 text-truncate">{server.total}</div>
												<div className="progress h-3px bg-white-transparent-2 mb-1">
													<div className="progress-bar bg-theme" style={{width: server.progress}}></div>
												</div>
												<div className="fs-11px text-white text-opacity-50 mb-2 text-truncate">
													{server.time}
												</div>
										...
										<CardBody className="py-2">
											<div className="d-flex align-items-center">
												...
												<div className="flex-1 ps-2">
													<table className="w-100 small mb-0 text-white text-opacity-60">
														<tbody>
												...
												<td>
													<div className="d-flex">
														...
														<div className="flex-1 ps-3">
															<div className="mb-1"><small className="fs-9px fw-500 lh-1 d-inline-block rounded-0 badge bg-white bg-opacity-25 text-white text-opacity-75 pt-5px">{product.sku}</small></div>
															<div className="fw-500 text-white">{product.title}</div>
															{product.price}
														</div>
													</div>
												</td>
													...
													<table className={((index < (productData.length - 1)) ? 'mb-2' : '')}>
														<tbody>
															<tr>
																...
																<td className="text-white text-opacity-75 fw-500">{product.qty}</td>
															</tr>
															<tr>
																<td className="pe-3">REVENUE:</td>
																<td className="text-white text-opacity-75 fw-500">{product.revenue}</td>
															</tr>
															<tr>
																<td className="pe-3 text-nowrap">PROFIT:</td>
																<td className="text-white text-opacity-75 fw-500">{product.profit}</td>
															</tr>
														</tbody>
													</table>
												</td>
												<td><a href="#/" className="text-decoration-none text-white"><i className="bi bi-search"></i></a></td>
											</tr>
										))) : (
											<tr>
											...
											<tr key={index}>
												<td>
													<span className="d-flex align-items-center">
														<i className={'bi bi-circle-fill fs-6px me-2 ' + ((log.highlight) ? 'text-theme' : 'text-white text-opacity-30')}></i> 
														{log.title}
													</span>
												</td>
												<td><small>{log.time}</small></td>
												<td><span className={'badge d-block rounded-0 pt-5px w-70px ' + ((log.highlight) ? 'bg-theme text-theme-900' : 'bg-white bg-opacity-25')} style={{ minHeight: '18px'}}>PRODUCT</span></td>
												<td><a href="#/" className="text-decoration-none text-white"><i className="bi bi-search"></i></a></td>
											</tr>
										))) : (
											<tr>
